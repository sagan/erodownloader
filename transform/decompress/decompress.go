...
func Transformer(tc *transform.TransformerContext) (changed bool, err error) {
	...
	for _, entry := range entries {
		if util.First(pathspec.GitIgnore(transform.IgnoredFilePatterns, entry.Name())) {
			continue
		}
		if entry.IsDir() {
			tc.Log(STR_STOP_NOT_EMPTY)
			return
		}
		if inputFile == "" {
	...
	if inputFile == "" || format == "" {
		tc.Log("no files to process")
		return
	}

	origdir := filepath.Join(tc.Dir, constants.ORIG_DIR)
	if util.FileExists(origdir) {
		entries, err = os.ReadDir(origdir)
		if err != nil || len(entries) > 0 {
			tc.Log("Folder is in inconsistent state: the .orig folder exists and can not be safely cleaned (%v)", err)
			return
		}
		os.RemoveAll(origdir)
	}

	tc.Log("Dir is of archive %s (%v)", inputFile, originalFiles)
	if filepath.Ext(inputFile) == EXT_EXE && format == EXT_RAR { // sfx
		newFile := inputFile[:len(inputFile)-len(EXT_EXE)] + EXT_RAR
		tc.Log("Rename %q to %q", inputFile, newFile)
		changed = true
		if util.FileExists(newFile) ||
			atomic.ReplaceFile(filepath.Join(tc.Dir, inputFile), filepath.Join(tc.Dir, newFile)) != nil {
			tc.Log("unable to rename .exe to .rar, abort")
			return
		}
	...
	inputFilePath := filepath.Join(tc.Dir, inputFile)
	tc.Log("Extracting %q to %s", inputFilePath, tmpdir)
	switch format {
	case EXT_ZIP:
		mode := util.ParseInt(tc.Options.Get("zipmode"), config.DEFAULT_ZIPMODE)
		err = ExtractZip(inputFilePath, tmpdir, tc.Options["password"], mode, tc.Log)
	default:
		_, _, _, err = xtractr.ExtractFile(&xtractr.XFile{
			FilePath:  inputFilePath,
			OutputDir: tmpdir,
			Passwords: tc.Options["password"],
			FileMode:  0600,
			DirMode:   0700,
		})
		// 目前 Go 对部分压缩文件格式支持不佳（例如 rar5 创建的自解压文件）。使用 7z 外部解压工具。
		if err != nil && tc.Options.Has("sevenzip_binary") {
			passwords := []string{}
			passwords = append(passwords, tc.Options["password"]...)
			if len(passwords) == 0 {
			...
			for _, password := range passwords {
				...
				cmd := exec.Command(tc.Options.Get("sevenzip_binary"), args...)
				var output []byte
				output, err = cmd.CombinedOutput()
				if err != nil {
					tc.Log("7z %v failed: %v. output=%s", args, err, string(output))
					if !strings.Contains(string(output), "Wrong password?") {
						break
					}
				} else {
					tc.Log("7z %v successed", args)
					break
				}
			}
		}
	...
	tc.Log("Extract done")
	changed = true
	if err = util.MakeCleanTmpDir(origdir); err != nil {
		return
	}
	for _, file := range originalFiles {
		tc.Log("move original %s => %s", filepath.Join(tc.Dir, file), filepath.Join(origdir, file))
		if err = atomic.ReplaceFile(filepath.Join(tc.Dir, file), filepath.Join(origdir, file)); err != nil {
			return
		}
	}
	contentDir := tmpdir
	...
	for {
		...
		if len(contentFiles) == 1 && contentFiles[0].IsDir() { // 去除压缩包里多层套娃文件夹结构
			tc.Log("Remove duplicate folder structure %q", contentFiles[0].Name())
	...
	for _, file := range originalFiles {
		if err = atomic.ReplaceFile(filepath.Join(origdir, file), helper.GetNewFilePath(tc.BackupDir, file)); err != nil {
			return
		}
	}
	os.RemoveAll(origdir)
...
func ExtractZip(inputFile, outputDir string, passwords []string, mode int, logger transform.Logger) (err error) {
	...
	logger("detected zip filename encoding: %s", encoding)
	if encoding == "UTF-8" {
		encoding = ""
	}
...
func DetectFilenamesEncoding(rawFilenames []string, mode int) (encoding string, possibleEncodings []string, err error) {
	...
	for _, rawFilaname := range rawFilenames {
		...
		results = util.FilterSlice(results, func(result chardet.Result) bool {
			return slices.Contains(constants.CjkCharsets, result.Charset)
		})
		if len(results) == 0 {
	...
	if encoding == "" {
		if possibleEncodings == nil {
			...
			if len(validEncodings) == 1 {
				...
				if mode == 1 {
					for _, encoding := range constants.CjkCharsets {
...
func init() {
	for i := range NoDecompressFilenames {
		NoDecompressFilenames[i] = strings.ToLower(NoDecompressFilenames[i])
	}
